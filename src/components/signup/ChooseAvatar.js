import React, { Component, } from 'react';
import {Redirect} from 'react-router-dom'
import Logo from '../global/Logo'
import defaultImage from '../../ressources/icons/logo PM.png'
import UploadProfilePic from '../global/UploadProfilePic'

import btnStyle from '../global/buttons.module.css'
import style from './ChooseAvatar.module.css'
import {UPLOAD_PIC} from '../../actions/rootActions'
import {getJwt, uploadImage, updateUserAvatar} from '../../helpers'

import {connect} from 'react-redux'

let avatars = new Array(5).fill(defaultImage)
avatars.push('data:image/jpeg;base64,')

class ChooseAvatar extends Component {
	
	constructor(props) {
		super(props)
		this.state = {
			index: null,
			isAvatar: true,
		}
	}

	async handleSubmit() {
		
		const jwt = getJwt()
		// console.log(jwt)

		//query store
		if(this.state.isAvatar) {
			console.log('avatar')
		}
		else if(this.props.profilePicPreviewBase64) {
			console.log('upload base 64')
			const url = await uploadImage(this.props.profilePicPreviewBase64)
			console.log(url)
			const userAvatar = await updateUserAvatar(url)
		}
		else {
			console.log('else')
		}

		//axios

		//redirect to onboarding
		// this.setState({redirect: true})
	}

	render() {

		return (
			<div className={style.ChooseAvatar}>
				<Logo/>

				<h2>Choisis un avatar pour commencer.</h2>
					
				<div onClick={() => this.setState({isAvatar: false})}>
					<UploadProfilePic image={avatars[this.state.index]}/>
				</div>

				<div className={style.avatarWrapper}>
					{avatars.map((oneAvatar, index) => (
						<div className='avatar-container' key={index} onClick={() => this.setState({index: index, isAvatar: true})}>
							<img className={style.avatarImg} src={oneAvatar} alt="avatar"/>
						</div>
						))}
				</div>

				<button className={btnStyle.bigBlue} onClick={() => this.handleSubmit()}>Valider</button>
				<button className={btnStyle.smallGrey}>Pas maintenant</button>

				{this.state.redirect ? <Redirect to='/onboarding' /> : null}
			</div>
		)
	}
}

const mapStateToProps = (state) => ({
	profilePicPreviewBase64: state.profilePicPreviewBase64,
})

const mapDispatchToProps = (dispatch) => ({
	upload: () => dispatch(UPLOAD_PIC()),
})


export default connect(mapStateToProps, mapDispatchToProps)(ChooseAvatar)